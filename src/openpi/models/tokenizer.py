...
class QwenTokenizer:
    """Prompt tokenizer for Qwen-backbone VLA variants (diffusion-backbone study
    Models C/D). Mirrors PaligemmaTokenizer's tokenize() contract but produces
    token ids that index Qwen's vocab, so a Qwen backbone's embed_tokens can
    consume them. Uses the same padding/truncation and the pi0-style "\\n"
    start-of-answer convention (state carried by the continuous action path).
    """

    def __init__(self, max_len: int = 200, repo_id: str = "Qwen/Qwen2.5-7B"):
    ...
    def tokenize(self, prompt: str, state: np.ndarray | None = None) -> tuple[np.ndarray, np.ndarray]:
        cleaned_text = prompt.strip().replace("_", " ").replace("\n", " ")
        if state is not None:
            # Pi05 discrete-state format (only if discrete_state_input=True).
            discretized_state = np.digitize(state, bins=np.linspace(-1, 1, 256 + 1)[:-1]) - 1
            state_str = " ".join(map(str, discretized_state))
            full_prompt = f"Task: {cleaned_text}, State: {state_str};\nAction: "
            tokens = self._tokenizer.encode(full_prompt)
        else:
            # Pi0/Pi05 continuous-state format: prompt + "\n" start-of-answer token.
            tokens = self._tokenizer.encode(cleaned_text) + self._tokenizer.encode("\n")
        tokens_len = len(tokens)
        if tokens_len < self._max_len:
            padding = [False] * (self._max_len - tokens_len)
            mask = [True] * tokens_len + padding
            ...
            if tokens_len > self._max_len:
                logging.warning(
                    f"Qwen token length ({tokens_len}) exceeds max length ({self._max_len}), truncating."
                )
            tokens = tokens[: self._max_len]
            mask = [True] * self._max_len

        return np.asarray(tokens, dtype=np.int32), np.asarray(mask)
