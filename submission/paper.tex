...
\begin{tikzpicture}[
  ...
  io/.style ={draw, rounded corners, fill=gray!8, align=center,
              minimum width=30mm, minimum height=7mm, inner sep=2pt},
  bb/.style ={draw=orange!75!black, line width=0.7pt, rounded corners,
              fill=orange!14, align=center, minimum width=32mm, minimum height=30mm},
  exp/.style={draw=blue!65!black, line width=0.7pt, rounded corners,
              fill=blue!10, align=center, minimum width=30mm, minimum height=18mm},
...
% backbone (the swap), vertically centred on the input stack
\coordinate (mid) at ($(lang.east)!0.5!(state.east)$);
\node[bb, right=13mm of mid] (bb)
  {\textbf{Backbone}\\[1pt]{\scriptsize\emph{(the only part}}\\{\scriptsize\emph{that changes)}}\\[4pt]
   \textbf{C:} Dream-7B\\{\scriptsize discrete diffusion}\\[3pt]
   \textbf{D:} Qwen2.5-7B\\{\scriptsize autoregressive}};
% shared standalone expert
\node[exp, right=15mm of bb] (exp)
  {\textbf{Standalone}\\\textbf{flow expert}\\[2pt]{\scriptsize DiT --- identical}\\{\scriptsize for C and D}};
% output
\node[out, right=10mm of exp] (out) {Action\\chunk $a_{1:H}$};
% arrows
\foreach \n in {img,lang,state,act}{\draw[->] (\n.east) -- (bb.west);}
\draw[->] (bb.east) -- node[above]{$H_{\mathrm{action}}$} (exp.west);
\draw[->] (exp.east) -- node[above]{$v_\theta$} (out.west);
\end{tikzpicture}%
}
